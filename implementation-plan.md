...
**Phase 6: Zip64 Support** (3-4 days)
- Update central directory parser for ZIP64 extra fields
- Update stream processor for ZIP64 local headers
- Update writer to generate ZIP64 structures
- Update writer's alignment logic to account for larger headers
- Test with large files (>4 GiB)

**Phase 7: Add upload support to writer** (2-3 days)
...
- Make BTRFS compression availability on extraction location explicit in downloader CLI args:
  --decompress-mode=[auto|btrfs-encoded-write|unencoded-write-only]
  Fail early if btrfs-encoded-write is requested but not available.
  Implement in-process zstandard decompression fallback if unencoded-write-only is requested but the frame is compressed.
